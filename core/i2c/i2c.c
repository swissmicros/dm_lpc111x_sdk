...
volatile uint32_t i2c_irq_cnt = 0;
...
//#else
  // Use 400kHz clock (max for PCF8563)
  // ... 100kHz ... quater the speed
  I2C_I2CSCLL   = current_clock/100000/2; //I2SCLL_SCLL; 
  I2C_I2CSCLH   = current_clock/100000/2; //I2SCLH_SCLH;
//#endif
...
**
*****************************************************************************/
#define I2C_IRQ_TIMEOUT 10 // systicks
uint32_t i2cEngine( void )
{
  ...
  /*
    ...
    or no i2c irq arrives for I2C_IRQ_TIMEOUT systicks.
  */
  uint32_t tout_tick = 0;
  uint32_t last_i2c_irq = i2c_irq_cnt - 1;
  while (I2CMasterState < 0x100) {
    if ( last_i2c_irq != i2c_irq_cnt ) {
      last_i2c_irq = i2c_irq_cnt;
      tout_tick = systickGetTicks() + I2C_IRQ_TIMEOUT;
    }
    if ( tout_tick == systickGetTicks() ) {
      I2CStop();
      return ( FALSE );
    }
  };
